...
Student admissions, courses, grades, fees
...
- **Database Name**: StudentInfoDB
- **ORM**: ADO.NET
- **Migration Tool**: SQL Scripts

### Backend
- **Framework**: MySQL, VB.NET
- **Language**: C# / VB.NET
...
```
08-Education-Student-Information-System/
├── README.md                          # This file
...
│
├── src/                               # Source code
│   ├── Education-Student-Information-System.sln            # Visual Studio solution
│   ├── Web/                          # Web application
...
cd 08-Education-Student-Information-System
```
...
sqlcmd -S localhost -d StudentInfoDB -i 02-CreateTables.sql
sqlcmd -S localhost -d StudentInfoDB -i 03-CreateStoredProcedures.sql
sqlcmd -S localhost -d StudentInfoDB -i 04-CreateTriggers.sql
sqlcmd -S localhost -d StudentInfoDB -i 05-CreateIndexes.sql
sqlcmd -S localhost -d StudentInfoDB -i 06-SeedData.sql
```

Or use the automated script:

```powershell
.\deployment\setup-database.ps1 -ServerName "localhost" -DatabaseName "StudentInfoDB"
```
...
```xml
<connectionStrings>
  <add name="StudentInfoDB" 
       connectionString="Server=localhost;Database=StudentInfoDB;Integrated Security=true;" 
...
```powershell
cd src
msbuild Education-Student-Information-System.sln /p:Configuration=Release
```
...
```powershell
.\deployment\setup-iis.ps1 -SiteName "Education-Student-Information-System" -Port 8080
```
...
<connectionStrings>
  <add name="StudentInfoDB" 
       connectionString="Server=YOUR_SERVER;Database=StudentInfoDB;User Id=YOUR_USER;Password=YOUR_PASSWORD;" 
       providerName="System.Data.SqlClient" />
</connectionStrings>
```
...
```xml
<appSettings>
  <add key="ApplicationName" value="Education-Student-Information-System" />
  <add key="PageSize" value="20" />
  <add key="SessionTimeout" value="30" />
  <add key="EnableLogging" value="true" />
  <add key="LogPath" value="C:\Logs\Education-Student-Information-System" />
</appSettings>
```
...
```bash
# Build Docker image
docker build -t Education-Student-Information-System:latest .
...
az deployment group create --resource-group rg-Education-Student-Information-System --template-file main.bicep
...
- **Logs**: Check `C:\Logs\Education-Student-Information-System`
...
SELECT * FROM sys.dm_exec_sessions WHERE database_id = DB_ID('StudentInfoDB')

-- Check long-running queries
SELECT * FROM sys.dm_exec_requests WHERE database_id = DB_ID('StudentInfoDB')
```
